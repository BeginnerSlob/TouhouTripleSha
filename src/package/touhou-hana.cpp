    ...
    virtual bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        if (triggerEvent == CardUsed) {
            ...
            if (str.endsWith("_slash"))
                str = "slash";
            if (room->askForCard(player, name, "@thhuaji:::" + str))
                return false;
...
public:
    ...
    {
        return TriggerSkill::triggerable(player) && player->getHp() == 1 && player->getPhase() == Player::Start
...
public:
    ...
    {
        ServerPlayer *target
            = room->askForPlayerChosen(player, room->getOtherPlayers(player), objectName(), "@thmopao", true, true);
        if (target) {
            room->broadcastSkillInvoke(objectName());
...
class ThYuwu : public TriggerSkill
{
public:
    ThYuwu()
        : TriggerSkill("thyuwu")
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        ...
        room->killPlayer(data.value<DamageStruct>().to);
        return false;
    }
};
...
class ThBianVS : public ZeroCardViewAsSkill
{
public:
    ThBianVS()
        : ZeroCardViewAsSkill("thbian")
    {
        response_pattern = "@@thbian";
    }

    virtual const Card *viewAs() const
    {
        Slash *slash = new Slash(Card::NoSuit, 0);
        slash->setSkillName("thbian");
        return slash;
    }
};
...
public:
    ...
    {
        events << HpRecover;
        view_as_skill = new ThBianVS;
    }

    virtual TriggerList triggerable(TriggerEvent, Room *, ServerPlayer *player, QVariant &) const
    {
        TriggerList skill_list;
        if (TriggerSkill::triggerable(player) && player->hasFlag("Global_Dying") && player->getHp() >= 1)
    ...
    virtual bool cost(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *) const
    {
        return room->askForUseCard(player, "@@thbian", "@thbian");
    }
};

class ThBianTargetMod : public TargetModSkill
{
public:
    ThBianTargetMod()
        : TargetModSkill("#thbian")
    {
    ...
    {
        if (card->getSkillName() == "thbian")
            return from->getMaxHp() - 1;
        return 0;
    }
...
public:
    ThYuchangTargetMod()
        : TargetModSkill("#thyuchang-target")
    {
    }
...
ThMimengDialog *ThMimengDialog::getInstance(const QString &object, bool left, bool right, bool play_only, bool slash_combined,
                                            bool delayed_tricks)
{
...
ThMimengDialog::ThMimengDialog(const QString &object, bool left, bool right, bool play_only, bool slash_combined,
                               bool delayed_tricks)
    : object_name(object)
    , play_only(play_only)
    , slash_combined(slash_combined)
    , delayed_tricks(delayed_tricks)
{
...
bool ThMimengDialog::isButtonEnabled(const QString &button_name) const
{
    ...
    if (allowings.isEmpty())
        return !ban_list.contains(card->getClassName()) && !Self->isCardLimited(card, Card::MethodUse, true)
...
QGroupBox *ThMimengDialog::createRight()
{
    ...
    QList<const Card *> cards = Sanguosha->findChildren<const Card *>();
    foreach (const Card *card, cards) {
        if (card->getTypeId() == Card::TypeTrick && (delayed_tricks || card->isNDTrick()) && !map.contains(card->objectName())
            && !ServerInfo.Extensions.contains("!" + card->getPackage())) {
            Card *c = Sanguosha->cloneCard(card->objectName());
...
public:
    ...
    {
        if (player->getHandcardNum() != 1 || pattern.startsWith(".") || pattern.startsWith("@"))
            return false;
...
void ThQuanshanGiveCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const
{
    ...
    CardMoveReason reason(CardMoveReason::S_REASON_GIVE, source->objectName(), targets.first()->objectName(), "thquanshan",
                          QString());
...
public:
    ThQuanshan()
        : ZeroCardViewAsSkill("thquanshan")
    {
    }
...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *) const
    {
        ...
        if (judge.isGood()) {
            ...
            Card *card = new ThQuanshanCard;
            if (room->isSomeonesTurn()) {
                ServerPlayer *current = room->getCurrent();
                if (card->targetFilter(QList<const Player *>(), current, player)
                    && player->askForSkillInvoke("thquanshan", QVariant::fromValue(current)))
                    room->useCard(CardUseStruct(card, player, current));
                else
                    delete card;
            } else
                delete card;
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == EventPhaseChanging) {
            ...
            if (use.card->isKindOf("BasicCard") || (use.card->isKindOf("TrickCard") && player->getHp() == 1)) {
                if (use.from && use.from->isAlive() && !(use.from->isNude() && player->isNude()) && use.to.contains(player))
                    skill_list.insert(player, QStringList(objectName()));
            }
        }
        return skill_list;
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        ...
        const Card *card = NULL;
        if (!player->isNude()) {
            card = room->askForCard(player, "..", "@thzheyin-give:" + use.from->objectName(), data, Card::MethodNone);
            if (card)
                room->moveCardTo(card, use.from, Player::PlaceHand,
                                 CardMoveReason(CardMoveReason::S_REASON_GIVE, player->objectName(), use.from->objectName(),
                                                objectName(), QString()));
        }

        if (!card && !use.from->isNude()) {
            card = room->askForCard(use.from, "..", "@thzheyin-give:" + player->objectName(), data, Card::MethodNone);
            if (card)
                room->moveCardTo(card, player, Player::PlaceHand,
                                 CardMoveReason(CardMoveReason::S_REASON_GIVE, use.from->objectName(), player->objectName(),
                                                objectName(), QString()));
        }
...
{
    CardMoveReason reason(CardMoveReason::S_REASON_GIVE, source->objectName(), targets.first()->objectName(), "thyachui",
                          QString());
    room->obtainCard(targets.first(), this, reason);
...
public:
    ...
    virtual bool triggerable(const ServerPlayer *player) const
    {
        return TriggerSkill::triggerable(player) && player->getPhase() == Player::Play && player->getMark("@stiff") > 0;
    }
...
void ThXianfaCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    QString choice
        = room->askForChoice(effect.from, "thxianfa", "start+judge+draw+discard+finish", QVariant::fromValue(effect.to));
        ...
        if (TriggerSkill::triggerable(target) && target->getPhase() == Player::Start) {
            ...
            foreach (ServerPlayer *p, room->getAllPlayers()) {
                if (!target->inMyAttackRange(p))
                    continue;
                has_target = true;
                if (qMax(0, p->getHp()) < target->getHp())
                    return false;
            }
            return has_target;
        }
...
ThShengzhiDiscardCard::ThShengzhiDiscardCard()
{
    target_fixed = true;
}

class ThShengzhiDiscard : public ViewAsSkill
{
public:
    ThShengzhiDiscard()
        : ViewAsSkill("thshengzhidiscard")
    {
        response_pattern = "@@thshengzhidiscard";
    }
    ...
    virtual const Card *viewAs(const QList<const Card *> &cards) const
    {
        ...
        Card *card = new ThShengzhiDiscardCard;
        card->addSubcards(cards);
        return card;
    }
};
...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *ask_who) const
    {
        if (!room->askForUseCard(player, "@@thshengzhidiscard", "@thshengzhi:" + ask_who->objectName(), -1,
                                 Card::MethodDiscard)) {
            ...
            static QMap<QString, Player::Phase> phase_map;
        ...
        if (card) {
            ...
            if ((!player->getJudgingArea().isEmpty() || player->getWeapon())
                ...
                CardsMoveStruct move(
                    card_ids, ask_who, Player::PlaceHand,
...
{
public:
    ThLiuzhenViewAsSkill()
        : ZeroCardViewAsSkill("thliuzhen")
    {
...
public:
    ...
    virtual bool cost(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *ask_who) const
    {
        if (triggerEvent == TargetSpecified) {
            ...
            foreach (ServerPlayer *tar, use.to)
                room->setPlayerFlag(tar, "-liuzhenold");
            return false;
        } else if (triggerEvent == SlashMissed && ask_who->askForSkillInvoke("thliuzhen_missed", QVariant::fromValue(player))) {
            LogMessage log;
    ...
    virtual bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *ask_who) const
    {
        if (triggerEvent == SlashMissed) {
            if (!player->canDiscard(player, "he") || room->askForChoice(ask_who, objectName(), "discard+draw", QVariant::fromValue(player)) == "draw")
...
{
public:
    ThTianchanViewAsSkill()
        : OneCardViewAsSkill("thtianchanv")
    {
    ...
    virtual bool isEnabledAtResponse(const Player *player, const QString &pattern) const
    {
        if (pattern != "peach" || player->getKingdom() != "hana")
            return false;
    ...
    : Package("touhou-hana")
{
    ...
    addMetaObject<ThShengzhiDiscardCard>();
    addMetaObject<ThLiuzhenCard>();

    skills << new ThMopao << new ThQuanshanGive << new ThShengzhiDiscard << new ThTianchanViewAsSkill;
}
