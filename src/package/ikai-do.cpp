...
IkXinqiViewAsSkill::IkXinqiViewAsSkill()
    : ZeroCardViewAsSkill("ikxinqi$")
{
}
...
{
    return hasKazeGenerals(player) && (pattern == "slash" || pattern == "@ikxinqi")
...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *liubei, QVariant &, ServerPlayer *) const
    {
        QList<ServerPlayer *> lieges = room->getLieges("kaze", liubei);
        foreach (ServerPlayer *liege, lieges) {
            const Card *slash = room->askForCard(liege, "slash", "@ikxinqi-slash:" + liubei->objectName(), QVariant(),
...
public:
    IkZhenhongTargetMod()
        : TargetModSkill("#ikzhenhong-target")
    {
    }
...
{
public:
    NonCompulsoryInvalidity()
        : InvaliditySkill("#non-compulsory-invalidity")
    {
    }
...
public:
    ...
    {
        return TriggerSkill::triggerable(target) && target->getMark("@yehua") == 0 && target->isWounded();
    }
...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *) const
    {
        ...
        foreach (ServerPlayer *liege, room->getLieges("hana", player)) {
            const Card *jink = room->askForCard(liege, "jink", "@ikhuanwei-jink:" + player->objectName(), tohelp,
...
public:
    ...
    virtual bool triggerable(const ServerPlayer *player) const
    {
        return TriggerSkill::triggerable(player) && !player->isNude();
    }
    ...
    virtual bool cost(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        ...
        prompt_list << "@iktiansuo-card" << judge->who->objectName() << objectName() << judge->reason
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent triggerEvent, Room *, ServerPlayer *xiahou, QVariant &data,
                                    ServerPlayer *&) const
    {
        QStringList skill;
        if (triggerEvent == FinishJudge) {
        ...
        if (!TriggerSkill::triggerable(xiahou))
            return skill;
        DamageStruct damage = data.value<DamageStruct>();
        for (int i = 0; i < damage.damage; i++)
            skill << objectName();
...
public:
    ...
    virtual bool cost(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        ...
        if (room->askForYiji(player, ids, objectName(), false, true, true, -1, QList<ServerPlayer *>(), CardMoveReason(),
                             "@ikqingjian-distribute", true)) {
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        ...
        if (ids.isEmpty())
            return false;
        while (room->askForYiji(player, ids, objectName(), false, false, true, -1, QList<ServerPlayer *>(), CardMoveReason(),
                                "@ikqingjian-distribute", false)) {
            room->notifySkillInvoked(player, objectName());
            if (player->isDead())
                return false;
        }
...
{
    if (targets.length() >= Self->getMark("ikchibao") || to_select->getHandcardNum() < Self->getHandcardNum()
        || to_select == Self)
        return false;
...
public:
    ...
    {
        if (!TriggerSkill::triggerable(zhangliao))
            return QStringList();
        QList<ServerPlayer *> targets;
...
public:
    IkChibaoAct()
        : TriggerSkill("#ikchibao")
    {
        events << AfterDrawNCards << EventPhaseChanging;
        frequency = Compulsory;
    }

    virtual QStringList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *zhangliao, QVariant &,
                                    ServerPlayer *&) const
    {
        if (triggerEvent == EventPhaseChanging) {
            foreach (ServerPlayer *p, room->getAlivePlayers())
        ...
        if (zhangliao->getMark("ikchibao") == 0)
            return QStringList();
        return QStringList(objectName());
    }
...
public:
    ...
    {
        if (!xuchu || xuchu->getMark("@nude") == 0 || xuchu->isDead())
            return QStringList();
        DamageStruct damage = data.value<DamageStruct>();
...
public:
    ...
    virtual bool cost(TriggerEvent, Room *room, ServerPlayer *guojia, QVariant &, ServerPlayer *) const
    {
        ServerPlayer *target = room->askForPlayerChosen(guojia, room->getAlivePlayers(), objectName(), "@iktiandu", true, true);
        if (target) {
            guojia->tag["IkTianduTarget"] = QVariant::fromValue(target);
            room->broadcastSkillInvoke(objectName());
            return true;
        }
        return false;
    ...
    {
        ServerPlayer *target = guojia->tag["IkTianduTarget"].value<ServerPlayer *>();
        guojia->tag.remove("IkTianduTarget");
        if (target) {
            JudgeStruct *judge = data.value<JudgeStruct *>();
...
IkYumeng::IkYumeng()
    : MasochismSkill("ikyumeng")
{
    frequency = Frequent;
    n = 2;
}

QStringList IkYumeng::triggerable(TriggerEvent, Room *, ServerPlayer *player, QVariant &data, ServerPlayer *&) const
{
    QStringList skills;
    if (TriggerSkill::triggerable(player)) {
        int times = data.value<DamageStruct>().damage;
...
bool IkYumeng::cost(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *) const
{
    if (player->askForSkillInvoke(objectName())) {
        room->broadcastSkillInvoke(objectName());
        return true;
...
void IkYumeng::onDamaged(ServerPlayer *guojia, const DamageStruct &) const
{
    guojia->drawCards(n, objectName());
    QList<int> card_ids = guojia->handCards();
    guojia->getRoom()->askForRende(guojia, card_ids, objectName(), false, true, 2);
}
...
void IkZhihengCard::onUse(Room *room, const CardUseStruct &card_use) const
{
    QList<int> hands = card_use.from->handCards();
...
void IkZhihengCard::use(Room *, ServerPlayer *source, QList<ServerPlayer *> &) const
{
    int n = subcardsLength();
    if (hasFlag("more"))
        ++n;
    if (source->isAlive())
        source->drawCards(n, "ikzhiheng");
}
...
public:
                                    ...
                                    ServerPlayer *&ask_who) const
    {
        ...
        CardUseStruct use = data.value<CardUseStruct>();
        if (!use.from || use.from->isDead() || use.from->getKingdom() != "yuki")
            return skill_list;

        if (use.card->isKindOf("Peach") && use.to.contains(use.from)) {
            ask_who = use.from;
            foreach (ServerPlayer *p, room->getOtherPlayers(player)) {
                if (p->hasLordSkill("ikjiyuan") && use.from->getHp() > qMax(0, p->getHp()))
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *sunquan, QVariant &data, ServerPlayer *ask_who) const
    {
        room->recover(sunquan, RecoverStruct(ask_who));
...
class IkBiju : public MaxCardsSkill
{
public:
    IkBiju()
        : MaxCardsSkill("ikbiju")
    {
        frequency = Compulsory;
    }
    ...
    virtual int getExtra(const Player *target) const
    {
        if (!target->hasSkill(objectName()))
            return 0;
        int flags = 0;
        if (target->hasFlag("IkBijuInPlayPhase0"))
            ++flags;
        if (target->hasFlag("IkBijuInPlayPhase1"))
            ++flags;
        if (target->hasFlag("IkBijuInPlayPhase2"))
            ++flags;
        return flags > 1 ? 0 : 4;
    }
};
...
public:
                                    ...
                                    ServerPlayer *&) const
    {
        if (room->isSomeonesTurn(lvmeng) && lvmeng->getPhase() == Player::Play) {
            ...
            if (card) {
                int n = card->getColor();
                room->setPlayerFlag(lvmeng, "IkBijuInPlayPhase" + QString::number(n));
            }
        }
        return QStringList();
    }
...
public:
    ...
    {
        if (TriggerSkill::triggerable(target) && target->getPhase() == Player::Finish
            && (target->getMark("IkPojianSuit") == 15 || target->getMark("IkPojianType") == 14)) {
            foreach (ServerPlayer *p, room->getAlivePlayers()) {
                foreach (const Card *c, p->getJudgingArea()) {
                    foreach (ServerPlayer *op, room->getOtherPlayers(p)) {
                        if (!target->isProhibited(op, c) && !op->containsTrick(c->objectName()))
                            return QStringList(objectName());
                    }
                }
                for (int i = 0; i < S_EQUIP_AREA_LENGTH; ++i) {
                    const EquipCard *equip = p->getEquip(i);
                    if (equip) {
                        foreach (ServerPlayer *op, room->getOtherPlayers(p)) {
                            if (!op->getEquip(i)) {
                                return QStringList(objectName());
    ...
    virtual bool cost(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *) const
    {
        ...
        foreach (ServerPlayer *p, room->getAlivePlayers()) {
            ...
            foreach (const Card *c, p->getJudgingArea()) {
                foreach (ServerPlayer *op, room->getOtherPlayers(p)) {
                    if (!player->isProhibited(op, c) && !op->containsTrick(c->objectName())) {
                        targets << p;
                        can_invoke = true;
                        break;
                    }
        ...
        Q_ASSERT(!targets.isEmpty());
        ServerPlayer *target = room->askForPlayerChosen(player, targets, objectName(), "@ikpojian", true, true);
        if (target) {
            player->tag["IkPojianTarget"] = QVariant::fromValue(target);
            room->broadcastSkillInvoke(objectName());
            return true;
        }
        return false;
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *) const
    {
        ServerPlayer *target = player->tag["IkPojianTarget"].value<ServerPlayer *>();
        player->tag.remove("IkPojianTarget");
        if (target) {
            QList<int> disabled_ids;
            foreach (const Card *c, target->getJudgingArea()) {
                bool can_invoke = false;
                foreach (ServerPlayer *op, room->getOtherPlayers(target)) {
                    if (!player->isProhibited(op, c) && !op->containsTrick(c->objectName())) {
                        can_invoke = true;
                        break;
                    }
...
public:
                                    ...
                                    ServerPlayer *&) const
    {
        if (triggerEvent == EventPhaseChanging) {
            if (data.value<PhaseChangeStruct>().to == Player::NotActive) {
                foreach (ServerPlayer *p, room->getAlivePlayers()) {
                    p->setMark("IkPojianSuit", 0);
                    p->setMark("IkPojianType", 0);
                }
            }
            ...
            if (card) {
                int suit_n = card->getSuit();
                int type_n = card->getTypeId();
                if (suit_n <= 3)
                    lvmeng->setMark("IkPojianSuit", lvmeng->getMark("IkPojianSuit") | (1 << suit_n));
                if (type_n > 0)
                    lvmeng->setMark("IkPojianType", lvmeng->getMark("IkPojianType") | (1 << type_n));
            }
        }
        return QStringList();
    }
...
{
public:
    IkXuanhuoViewAsSkill()
        : OneCardViewAsSkill("ikxuanhuo")
    {
        filter_pattern = ".!";
...
public:
    ...
    {
        if (!TriggerSkill::triggerable(daqiao))
            return QStringList();
        CardUseStruct use = data.value<CardUseStruct>();
...
public:
    ...
    {
        if (!TriggerSkill::triggerable(luxun))
            return QStringList();
        CardsMoveOneTimeStruct move = data.value<CardsMoveOneTimeStruct>();
...
{
public:
    IkLingshiliTargetMod()
        : TargetModSkill("#iklingshili-target")
    {
    }
...
public:
    ...
    virtual bool cost(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *ask_who) const
    {
        if (player->getMark("@wayward") > 0) {
            ...
            ServerPlayer *target
                = room->askForPlayerChosen(ask_who, room->getOtherPlayers(ask_who), objectName(), "@ikguijiao", true, true);
            if (target) {
...
public:
    IkGuijiaoMaxCards()
        : MaxCardsSkill("#ikguijiao-maxcard")
    {
    }
