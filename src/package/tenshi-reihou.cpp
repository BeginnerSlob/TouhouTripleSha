...
public:
    RhDuanlongVS()
        : ZeroCardViewAsSkill("rhduanlong")
    {
    }
...
{
public:
    RhHonghuangTargetMod()
        : TargetModSkill("#rhhonghuang")
    {
        pattern = "TrickCard";
    }
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent, Room *, ServerPlayer *player, QVariant &data, ServerPlayer *&) const
    {
        if (TriggerSkill::triggerable(player)) {
            ...
            if ((use.card->isKindOf("BasicCard") || use.card->isNDTrick()) && use.to.contains(player))
                return QStringList(objectName());
        }
        return QStringList();
    }
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent, Room *, ServerPlayer *, QVariant &data, ServerPlayer *&) const
    {
        ...
        if (move.from_places.contains(Player::PlaceTable) && move.to_place == Player::DiscardPile && card
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *, QVariant &data, ServerPlayer *) const
    {
        ...
        if (card) {
            ...
            if (!ids.isEmpty()) {
                ...
                ServerPlayer *target
                    = room->askForPlayerChosen(player, room->getAllPlayers(), "rhliufu", "@rhliufu", false, true);
                DummyCard dummy(ids);
                CardMoveReason reason(CardMoveReason::S_REASON_GIVE, player->objectName(), target->objectName(), "rhliufu",
                                      QString());
...
public:
    ...
    virtual bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *ask_who) const
    {
        if (triggerEvent == EventAcquireSkill) {
            ServerPlayer *target
                = room->askForPlayerChosen(ask_who, room->getOtherPlayers(ask_who), objectName(), "@rhbaiming", false, true);
            room->broadcastSkillInvoke(objectName());
...
public:
    RhChengfeng()
        : OneCardViewAsSkill("rhchengfeng")
    {
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent, Room *, ServerPlayer *, QVariant &data, ServerPlayer *&ask_who) const
    {
        ...
        if (effect.from && effect.from->isAlive() && effect.from->hasSkill("rhzhengyang")
...
public:
    RhZhengyangProhibit()
        : ProhibitSkill("#rhzhengyang-prohibit")
    {
    }
...
public:
    ...
    virtual bool cost(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *ask_who) const
    {
        if (triggerEvent == EventAcquireSkill) {
            ...
            CardUseStruct use = data.value<CardUseStruct>();
            if (use.from == ask_who) {
                ServerPlayer *target = room->askForPlayerChosen(ask_who, room->getOtherPlayers(ask_who), objectName(),
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == ConfirmDamage) {
            if (damage.card && damage.card->getTypeId() != Card::TypeSkill && damage.card->isRed()
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *ask_who) const
    {
        ...
        try {
            foreach (ServerPlayer *p, room->getAllPlayers()) {
                if (p->canSlash(player, false)
                    && room->askForUseSlashTo(p, player, "@rhshenguang:" + player->objectName(), false)) {
...
{
    CardMoveReason reason(CardMoveReason::S_REASON_GIVE, effect.from->objectName(), effect.to->objectName(), "rhyinren",
                          QString());
    Room *room = effect.from->getRoom();
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == PreDamageDone) {
            ...
            if (player->getPhase() == Player::Discard && !player->hasFlag("RhSanglvDamageInPlayPhase")
...
public:
    ...
    {
        ServerPlayer *target
            = room->askForPlayerChosen(player, room->getOtherPlayers(player), objectName(), "@rhshenluo", true, true);
        if (target) {
            room->broadcastSkillInvoke(objectName());
...
const Card *RhYoushengCard::validate(CardUseStruct &card_use) const
{
    ...
    QList<const DelayedTrick *> cards = Sanguosha->findChildren<const DelayedTrick *>();
    foreach (const Card *c, cards) {
        if (!c->isKindOf("Lightning") && !choices.contains(c->objectName()) && !target->containsTrick(c->objectName())) {
            Card *trick = Sanguosha->cloneCard(c->objectName());
            trick->addSubcard(this);
            trick->setSkillName("rhyousheng");
                        ...
                        ServerPlayer *ask_who) const
    {
        ...
        if (triggerEvent == BeforeCardsMove) {
            ...
            while (!ids.isEmpty()) {
                ...
                ServerPlayer *target
                    = room->askForPlayerChosen(player, room->getOtherPlayers((ServerPlayer *)move.from), objectName(),
...
void RhYizhiCard::onUse(Room *room, const CardUseStruct &card_use) const
{
    ...
    CardMoveReason reason(CardMoveReason::S_REASON_PUT, source->objectName(), targets.last()->objectName(), "rhyizhi",
                          QString());
...
public:
    RhChenshengVS()
        : ZeroCardViewAsSkill("rhchensheng")
    {
    }
...
void RhNajieCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const
{
    CardMoveReason reason(CardMoveReason::S_REASON_REMOVE_FROM_PILE, QString(), "rhnajie", QString());
    room->throwCard(this, reason, source);
}

class RhNajieVS : public OneCardViewAsSkill
{
public:
    RhNajieVS()
        : OneCardViewAsSkill("rhnajie")
    {
        expand_pile = "tea";
        filter_pattern = ".|.|.|tea";
        response_pattern = "@@rhnajie";
    }

    virtual const Card *viewAs(const Card *originalCard) const
    {
        Card *card = new RhNajieCard;
        card->addSubcard(originalCard);
        return card;
    }
};

class RhNajie : public TriggerSkill
{
public:
    RhNajie()
        : TriggerSkill("rhnajie")
    {
        events << EventAcquireSkill << TargetSpecified;
        view_as_skill = new RhNajieVS;
    }
    ...
    {
        if (triggerEvent == EventAcquireSkill) {
            const Card *dummy = room->askForExchange(ask_who, objectName(), 998, 1, false, "@rhnajie-invoke", true);
            if (dummy) {
                room->sendCompulsoryTriggerLog(ask_who, objectName());
                room->broadcastSkillInvoke(objectName());
            ...
            return room->askForUseCard(ask_who, "@@rhnajie", "@rhnajie", -1, Card::MethodNone);
...
class RhNajieClear : public TriggerSkill
{
public:
    RhNajieClear()
        : TriggerSkill("#rhnajie")
...
public:
                                    ...
                                    ServerPlayer *&) const
    {
        ...
        if (TriggerSkill::triggerable(player) && player->getPhase() == Player::Play && player->getMark(objectName()) == 0) {
            CardUseStruct use = data.value<CardUseStruct>();
...
public:
    ...
    {
        ServerPlayer *target
            = room->askForPlayerChosen(player, room->getAlivePlayers(), objectName(), "@rhzhangchi", true, true);
        if (target) {
            room->broadcastSkillInvoke(objectName());
            player->tag[objectName()] = QVariant::fromValue(target);
            return true;
...
public:
    RhZhangchiVS()
        : ZeroCardViewAsSkill("rhzhangchiv")
    {
        attached_lord_skill = true;
    }
...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *ask_who) const
    {
        ...
        room->askForUseCard(player, "@@rhqiaozou",
                            QString("@rhqiaozou:%1::%2").arg(ask_who->objectName()).arg(use.card->objectName()));
    ...
    : Package("tenshi-reihou")
{
    ...
    related_skills.insertMulti("rhnajie", "#rhnajie");
