...
int Player::getAttackRange(bool include_weapon) const
{
    ...
    int extra_range = getMark("thhuanlong1") + getMark("threnmorange");
...
int Player::distanceTo(const Player *other, int distance_fix) const
{
    ...
    if ((hasSkill("thguimen") && other->isChained()) || (other->hasSkill("thguimen") && isChained()))
        return 1;
...
bool Player::hasSkill(const QString &skill_name, bool include_lose) const
{
    ...
    return skills.contains(skill_name) || acquired_skills.contains(skill_name) || extra_skills.contains(skill_name);
}
...
bool Player::canDiscard(const Player *to, const QString &flags) const
{
    ...
    if (this->isDead() || to->isDead())
        return false;
    if (flags.contains(handcard_flag) && !to->isKongcheng())
        return true;
    ...
    if (getGeneralName() == "anjiang" || getGeneral2Name() == "anjiang") {
        ...
        if (!basara.isEmpty()) {
            ...
            foreach (QString basara_gen, basaras) {
                ...
                if (general)
                    basara_list.append(general->getVisibleSkillList());
            }
        }
    }
