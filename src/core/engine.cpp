...
Engine::Engine()
{
    ...
    lua = CreateLuaState();
    if (!DoLuaScript(lua, "lua/config.lua"))
        exit(1);
    ...
    if (!DoLuaScript(lua, "lua/sanguosha.lua"))
        exit(1);
...
void Engine::addSkills(const QList<const Skill *> &all_skills)
{
    foreach (const Skill *skill, all_skills) {
        ...
        if (skill->inherits("ProhibitSkill"))
            ...
            else if (trigger_skill && trigger_skill->getTriggerEvents().contains(GameStart))
...
int Engine::getGeneralCount(bool include_banned, const QString &kingdom) const
{
    ...
    while (itor.hasNext()) {
        ...
        if (getBanPackages().contains(general->getPackage()))
            ...
            isBanned = true;
        else if (ServerInfo.EnableBasara && Config.value("Banlist/Basara").toStringList().contains(general->objectName()))
            isBanned = true;
        else if (ServerInfo.EnableHegemony && Config.value("Banlist/Hegemony").toStringList().contains(general->objectName()))
            isBanned = true;
...
QStringList Engine::getRoleList(const QString &mode) const
{
    ...
    for (int i = 0; QChar(roles[i]) != QChar('\0'); i++) {
        QString role;
...
QStringList Engine::getRandomLords() const
{
    ...
    foreach (QString nonlord, generals.keys()) {
        if (isGeneralHidden(nonlord) || lord_list.contains(nonlord))
            continue;
...
{
    if (player->hasSkill("thyanmeng") || player->hasSkill("thxuanyan"))
        return true;
