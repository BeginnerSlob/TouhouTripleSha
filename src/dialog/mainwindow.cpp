...
public:
    ...
    virtual void resizeEvent(QResizeEvent *event)
    {
        ...
        if (scene()->inherits("RoomScene")) {
            ...
            QRectF newSceneRect(-event->size().width() / 2, -event->size().height() / 2, event->size().width(),
                                event->size().height());
...
void MainWindow::on_actionExit_triggered()
{
    ...
    result = QMessageBox::question(this, tr("TouhouTripleSha"), tr("Are you sure to exit?"),
...
void MainWindow::on_actionReplay_triggered()
{
    ...
    QString filename = QFileDialog::getOpenFileName(this, tr("Select a replay file"), location,
...
void MainWindow::on_actionAbout_triggered()
{
    ...
    content.append(
        tr("Current version: %1 %2 (%3)<br/>").arg(Sanguosha->getVersion()).arg(config).arg(Sanguosha->getVersionName()));
...
{
    QString filename
        = QFileDialog::getOpenFileName(this, tr("Please select a replay file"), Config.value("LastReplayDir").toString(),
                                       tr("Pure text replay file (*.txt);; Image replay file (*.png)"));
...
{
    QString location = QStandardPaths::writableLocation(QStandardPaths::HomeLocation);
    QString filename = QFileDialog::getOpenFileName(this, tr("Load replay record"), location,
                                                    tr("Pure text replay file (*.txt);; Image replay file (*.png)"));
    ...
    foreach (PlayerRecordStruct *rec, record_map.values()) {
        ...
        item = new QTableWidgetItem;
        bool is_win
            = record->getRecordWinners().contains(rec->m_role) || record->getRecordWinners().contains(record_map.key(rec));
...
{
    QString content = tr("The GNU General Public License is the most widely used free software license, which guarantees end "
                         "users the freedoms to use, study, share, and modify the software.");
        ...
        if (!versions.contains(Sanguosha->getVersionNumber()) || versions[Sanguosha->getVersionNumber()].url.isEmpty()) {
            QMessageBox::warning(this, tr("New Version Available"),
                                 tr("There is a new version for TouhouTripleSha<br/> \
                                     ...
                                     .arg(versions[newest].versionNumber)
                                     .arg(versions[newest].updateDate),
            ...
            if (QMessageBox::question(this, tr("New Version Available"),
                                      tr("There is a new version for TouhouTripleSha<br/>"
                                         "The version number is %1<br/>"
                                         "Update date is %2<br/>"
                                         "Would you wanna download now?")
                                          .arg(vs.versionNumber)
                                          .arg(vs.updateDate))
                == QMessageBox::Yes) {
                QMessageBox::warning(this, tr("Tips"), tr("The new version will be downloading<br/>"
...
void MainWindow::httpFinished2()
{
    ...
    if (reply2->error()) {
        file->remove();
        QMessageBox::information(this, tr("Download New Version"), tr("Download failed: %1.").arg(reply2->errorString()));
    } else if (!redirectionTarget.isNull()) {
        QMessageBox::warning(this, tr("Download New Version"), tr("Redirect Error!"));
        return;
    } else {
#ifdef Q_OS_WIN
        if (QMessageBox::question(this, tr("Download New Version"), tr("Download Finish!<br/>"
...
#endif
            QMessageBox::warning(this, tr("Download New Version"), tr("The update package has been saved at root folder"));
    }
