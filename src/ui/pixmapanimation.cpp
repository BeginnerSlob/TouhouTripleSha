...
PixmapAnimation *PixmapAnimation::GetPixmapAnimation(QGraphicsItem *parent, const QString &emotion)
{
    ...
    if (pma->valid()) {
        if (emotion == "no-success") {
            ...
            pma->moveBy(pma->boundingRect().width() * 0.4, pma->boundingRect().height() * 0.3);
            pma->setScale(0.4);
        } else if (emotion == "effects/god_salvation") {
            pma->moveBy(pma->boundingRect().width() * 0.2, pma->boundingRect().height() * 0.2);
            pma->setScale(0.6);
        } else if (emotion == "effects/amazing_grace") {
            pma->moveBy(-pma->boundingRect().width() * 0.9, -pma->boundingRect().height() * 0.9);
            pma->setScale(2.8);
            ...
            pma->moveBy(-pma->boundingRect().width() * 0.35, -pma->boundingRect().height() * 0.3);
            pma->setScale(1.8);
        } else if (emotion == "effects/snatch") {
            pma->moveBy(pma->boundingRect().width() * 0.25, pma->boundingRect().height() * 0.25);
            pma->setScale(0.5);
        } else if (emotion == "effects/collateral_slash") {
            pma->moveBy(pma->boundingRect().width() * 0.4, pma->boundingRect().height() * 0.2);
            pma->setScale(0.6);
        } else if (emotion == "effects/lightning") {
            pma->moveBy(-pma->boundingRect().width() * 0.15, -pma->boundingRect().height() * 0.25);
            pma->setScale(1.8);
            ...
            pma->moveBy(pma->boundingRect().width() * 0.1, pma->boundingRect().height() * 0.1);
            pma->setScale(0.8);
        } else if ((emotion.startsWith("effects/") && emotion.contains("slash")) || (emotion.endsWith("damage/normal") || emotion.endsWith("damage/heavy"))) {
            pma->moveBy(pma->boundingRect().width() * 0.2, pma->boundingRect().height() * 0.2);
            pma->setScale(0.6);
        ...
        if (emotion != "effects/burning_camps")
            pma->moveBy((parent->boundingRect().width() - pma->boundingRect().width()) * 0.5, (parent->boundingRect().height() - pma->boundingRect().height()) * 0.5);
