...
BubbleChatBox::BubbleChatBox(const QRect &area, QGraphicsItem *parent)
    ...
    , backgroundPixmap("image/system/bubble.png")
    , rect(backgroundPixmap.rect())
    , area(area)
    , chatLabel(new BubbleChatLabel(this))
...
void BubbleChatBox::setText(const QString &text)
{
    ...
    chatLabel->setPos(QPointF(BoxLeftFrameWidth, rect.center().y() - (height * lineCount) + (lineCount - 1) * (height / 2) - (imageCount > 0 ? 1 : 0)));
