...
void GuanxingBox::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    for (int i = 0; i < itemCount; ++i) {
        ...
        painter->drawPixmap(top_rect, G_ROOM_SKIN.getPixmap(QSanRoomSkin::S_SKIN_KEY_CHOOSE_CARD_BOX_DEST_PLACE));
        if (!up_only) {
            IQSanComponentSkin::QSanSimpleTextFont font = G_COMMON_LAYOUT.m_chooseCardBoxDestPlaceFont;
            ...
            painter->drawPixmap(bottom_rect, G_ROOM_SKIN.getPixmap(QSanRoomSkin::S_SKIN_KEY_CHOOSE_CARD_BOX_DEST_PLACE));
