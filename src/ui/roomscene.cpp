...
void RoomScene::updateTargetsEnablity(const Card *card){
    ...
    while(itor.hasNext()){
        ...
        bool enabled = (card == NULL) || (!Sanguosha->isProhibited(Self, player, card) && maxVotes > 0);
...
void RoomScene::addSkillButton(const Skill *skill, bool from_left){
    ...
    if (btn->getViewAsSkill() != NULL  && !m_replayControl)
    {
        ...
        if (btn->getViewAsSkill()->objectName() == "thxingxie")
...
void RoomScene::updateSkillButtons(){
    foreach(const Skill* skill, Self->getVisibleSkillList(false)){
...
void RoomScene::onGameStart(){
    ...
    for(int i = 1; ; i++)
    {
        QString effect_file = QString("audio/bgm/%1.ogg").arg(QString::number(i));
        if(QFile::exists(effect_file))
            bgms << effect_file;
        else
            break;
    }
    QString bgmusic_path = bgms[qrand() % bgms.length()];
