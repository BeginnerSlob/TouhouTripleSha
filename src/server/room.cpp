    ...
    , _m_roomState(false)
{
    ...
    m_takeAGargs = QVariant(JsonArray());
}
...
void Room::enterDying(ServerPlayer *player, DamageStruct *reason)
{
    ...
    JsonArray args;
    args << (int)QSanProtocol::S_GAME_EVENT_PLAYER_DYING;
    ...
    if (player->isAlive()) {
        JsonArray args;
        args << (int)QSanProtocol::S_GAME_EVENT_PLAYER_QUITDYING;
        args << player->objectName();
        doBroadcastNotify(QSanProtocol::S_COMMAND_LOG_EVENT, args);
    }
        ...
        if (expose_roles) {
            foreach (ServerPlayer *player, m_alivePlayers) {
                if (Config.EnableHegemony) {
                    ...
                    if (role == "god")
                        role = Sanguosha->getGeneral(player->property("basara_generals").toString().split("+").at(0))
                                   ->getKingdom();
...
{
    JsonArray args;
    args << (int)QSanProtocol::S_GAME_EVENT_JUDGE_RESULT;
    args << judge->card->getEffectiveId();
    args << judge->isEffected();
    args << judge->who->objectName();
    args << judge->reason;
    doBroadcastNotify(QSanProtocol::S_COMMAND_LOG_EVENT, args);
}
...
void Room::gameOver(const QString &winner, bool isSurrender)
{
    ...
    if (isNormalGameMode(ServerInfo.GameMode)) {
        ...
        record << time.toString("yyyy-MM-dd hh:mm:ss") << ServerInfo.GameMode;
    ...
    JsonArray args;
    args << winner;
    args << toJsonArray(all_roles);
    doBroadcastNotify(S_COMMAND_GAME_OVER, args);
    throw GameFinished;
}
                                 ...
                                 bool extra_only)
{
    ...
    if (skill && skill->isVisible()) {
        JsonArray args;
        args << QSanProtocol::S_GAME_EVENT_DETACH_SKILL;
        args << player->objectName();
        args << skill_name;
        doBroadcastNotify(QSanProtocol::S_COMMAND_LOG_EVENT, args);
...
void Room::handleAcquireDetachSkills(ServerPlayer *player, const QStringList &skill_names, bool acquire_only, bool extra_only)
{
...
void Room::handleAcquireDetachSkills(ServerPlayer *player, const QString &skill_names, bool acquire_only, bool extra_only)
{
...
ServerPlayer *Room::doBroadcastRaceRequest(QList<ServerPlayer *> &players, QSanProtocol::CommandType command, time_t timeOut,
...
bool Room::askForSkillInvoke(ServerPlayer *player, const QString &skill_name, const QVariant &data)
{
    ...
    if (ai) {
        ...
        if (data.type() == QVariant::String)
            skillCommand = JsonArray() << skill_name << data.toString();
        else {
            ServerPlayer *p = data.value<ServerPlayer *>();
            QString data_str;
            if (p != NULL)
                data_str = "playerdata:" + p->objectName();
            skillCommand = JsonArray() << skill_name << data_str;
        }
        ...
        if (!doRequest(player, S_COMMAND_INVOKE_SKILL, skillCommand, true)) {
            ...
            if (isBool(clientReply))
                invoked = clientReply.toBool();
        }
    }

    if (invoked) {
        QVariant msg = JsonArray() << skill_name << player->objectName();
...
bool Room::_askForNullification(const Card *trick, ServerPlayer *from, ServerPlayer *to, bool positive,
                                _NullificationAiHelper aiHelper)
{
    tryPause();
    ...
    if (!validHumanPlayers.isEmpty()) {
        ...
        repliedPlayer
            = doBroadcastRaceRequest(validHumanPlayers, S_COMMAND_NULLIFICATION, timeOut, &Room::verifyNullificationResponse);
    }
    const Card *card = NULL;
    if (repliedPlayer != NULL) {
        JsonArray clientReply = repliedPlayer->getClientReply().value<JsonArray>();
        if (clientReply.size() > 0 && isString(clientReply[0]))
    ...
    if (trick->isKindOf("ExNihilo") || trick->isKindOf("AmazingGrace") || trick->isKindOf("GodSalvation")
        || trick->isKindOf("Nullification") || trick->isKindOf("LureTiger") || trick->isKindOf("Reinforce"))
    ...
    if (result) {
        if (card && card->getSkillName() == "jade") {
            ...
            if (ask_jade) {
                ...
                setPlayerProperty(repliedPlayer, "jade_trick_from", from->objectName());
                           ...
                           bool handcard_visible, Card::HandlingMethod method, const QList<int> &disabled_ids)
{
    ...
    if (who != player && !handcard_visible && player->getAI()
        ...
        if (ai) {
            ...
            args << flags;
            args << reason;
            args << handcard_visible;
            args << (int)method;
            args << toJsonArray(disabled_ids);
            bool success = doRequest(player, S_COMMAND_CHOOSE_CARD, args, true);
            //@todo: check if the card returned is valid
            QVariant clientReply = player->getClientReply();
            if (!success || !isNumber(clientReply)) {
    ...
    QVariant decisionData = QVariant::fromValue(
        QString("cardChosen:%1:%2:%3:%4").arg(reason).arg(card_id).arg(player->objectName()).arg(who->objectName()));
    thread->trigger(ChoiceMade, this, player, decisionData);
    return card_id;
}
...
const Card *Room::askForCard(ServerPlayer *player, const QString &pattern, const QString &prompt, const QVariant &data,
                             Card::HandlingMethod method, ServerPlayer *to, bool isRetrial, const QString &skill_name,
                             bool isProvision)
{
    ...
    if (has_provided || !player->isAlive()) {
        ...
        if (ai) {
            ...
            args << prompt;
            args << int(method);
            bool success = doRequest(player, S_COMMAND_RESPONSE_CARD, args, true);
    ...
    if (card) {
        ...
        QVariant decisionData
            = QVariant::fromValue(QString("cardResponded:%1:%2:_%3_").arg(pattern).arg(prompt).arg(card->toString()));
        thread->trigger(ChoiceMade, this, player, decisionData);
        ...
        if ((method == Card::MethodUse || method == Card::MethodResponse) && !isRetrial) {
            ...
            if (!card_ids.isEmpty()) {
                ...
                if (method == Card::MethodUse) {
                    CardMoveReason reason(CardMoveReason::S_REASON_LETUSE, player->objectName(), QString(),
                                          card->getSkillName(), QString());
...
const Card *Room::askForUseSlashTo(ServerPlayer *slasher, ServerPlayer *victim, const QString &prompt, bool distance_limit,
                                   bool disable_extra, bool addHistory)
{
...
const Card *Room::askForCardShow(ServerPlayer *player, ServerPlayer *requestor, const QString &reason)
{
    ...
    if (ai)
        ...
        if (player->getHandcardNum() == 1)
            ...
            bool success = doRequest(player, S_COMMAND_SHOW_CARD, QVariant(requestor->getGeneralName()), true);
            JsonArray clientReply = player->getClientReply().value<JsonArray>();
            if (success && !clientReply.isEmpty() && isString(clientReply[0]))
                card = Card::Parse(clientReply[0].toString());
            if (card == NULL)
...
const Card *Room::askForSinglePeach(ServerPlayer *player, ServerPlayer *dying)
{
    ...
    if (ai)
        ...
        JsonArray args;
        args << dying->objectName();
        args << peaches;
        bool success = doRequest(player, S_COMMAND_ASK_PEACH, args, true);
        JsonArray clientReply = player->getClientReply().value<JsonArray>();
        if (!success || clientReply.isEmpty() || !isString(clientReply[0]))
            return NULL;
    ...
    if (card) {
        QVariant decisionData = QVariant::fromValue(
            QString("peach:%1:%2:%3").arg(dying->objectName()).arg(1 - dying->getHp()).arg(card->toString()));
        thread->trigger(ChoiceMade, this, player, decisionData);
        result = card;
...
QString Room::askForTriggerOrder(ServerPlayer *player, const QString &reason, QMap<const ServerPlayer *, QStringList> &skills,
...
void Room::addPlayerHistory(ServerPlayer *player, const QString &key, int times)
{
    ...
    if (player)
        doNotify(player, S_COMMAND_ADD_HISTORY, args);
    else
        doBroadcastNotify(S_COMMAND_ADD_HISTORY, args);
}
...
void Room::setPlayerCardLimitation(ServerPlayer *player, const QString &limit_list, const QString &pattern, bool single_turn)
{
    ...
    args << pattern;
    args << single_turn;
    doNotify(player, S_COMMAND_CARD_LIMITATION, args);
}
...
void Room::setCardFlag(int card_id, const QString &flag, ServerPlayer *who)
{
    ...
    args << flag;
    if (who)
        doNotify(who, S_COMMAND_CARD_FLAG, args);
    else
        doBroadcastNotify(S_COMMAND_CARD_FLAG, args);
}
...
void Room::setFixedDistance(Player *from, const Player *to, int distance)
{
    ...
    args << distance;
    args << true;
    doBroadcastNotify(S_COMMAND_FIXED_DISTANCE, args);
}
...
void Room::insertAttackRangePair(Player *from, const Player *to)
{
    ...
    args << to->objectName();
    args << true;
    doBroadcastNotify(S_COMMAND_ATTACK_RANGE, args);
}
...
const ProhibitSkill *Room::isProhibited(const Player *from, const Player *to, const Card *card,
...
void Room::prepareForStart()
{
    if (scenario) {
        ...
        if (owner && owner->isOnline()) {
            ...
            JsonArray clientReply = owner->getClientReply().value<JsonArray>();
            if (!success || clientReply.isEmpty() || clientReply.size() != 2) {
                if (Config.RandomSeat)
                    qShuffle(m_players);
                assignRoles();
            } else if (Config.FreeAssignSelf) {
                QString name = clientReply[0].value<JsonArray>()[0].toString();
                QString role = clientReply[1].value<JsonArray>()[0].toString();
                ...
                JsonArray names = clientReply[0].value<JsonArray>();
                JsonArray roles = clientReply[1].value<JsonArray>();
                for (int i = 0; i < names.size(); i++) {
                    QString name = names[i].toString();
                    QString role = roles[i].toString();
...
bool Room::trustCommand(ServerPlayer *player, const QVariant &)
{
    player->acquireLock(ServerPlayer::SEMA_MUTEX);
    if (player->isOnline()) {
...
bool Room::pauseCommand(ServerPlayer *player, const QVariant &arg)
{
    if (!canPause(player))
        return false;
...
bool Room::processRequestCheat(ServerPlayer *player, const QVariant &arg)
{
    if (!Config.EnableCheat)
        return false;
    if (!arg.canConvert<JsonArray>() || !isNumber(arg.value<JsonArray>()[0]))
        return false;
...
bool Room::processRequestSurrender(ServerPlayer *player, const QVariant &)
{
    ...
    if (packet.parse(request)) {
        ...
        if (game_finished) {
            if (player && player->isOnline())
                doNotify(player, S_COMMAND_WARN, QVariant("GAME_OVER"));
            return;
        }
...
bool Room::toggleReadyCommand(ServerPlayer *, const QVariant &)
{
...
void Room::chooseGenerals(QList<ServerPlayer *> players)
{
    ...
    foreach (ServerPlayer *player, to_assign) {
        ...
        if (!player->m_isClientResponseReady || !isString(generalName)
            || !_setPlayerGeneral(player, generalName.toString(), true))
    ...
    if (Config.Enable2ndGeneral) {
        ...
        foreach (ServerPlayer *player, to_assign) {
            ...
            QVariant generalName = player->getClientReply();
            if (!player->m_isClientResponseReady || !isString(generalName)
                || !_setPlayerGeneral(player, generalName.toString(), false))
...
void Room::run()
{
    ...
    if (using_countdown) {
        ...
        doBroadcastNotify(S_COMMAND_START_IN_X_SECONDS, QVariant());
...
void Room::processResponse(ServerPlayer *player, const Packet *packet)
{
    ...
    if (player == NULL)
        ...
        emit room_message(
            tr("Reply command should be %1 instead of %2").arg(player->m_expectedReplyCommand).arg(packet->getCommandType()));
    else if (packet->localSerial != player->m_expectedReplySerial)
        emit room_message(
            tr("Reply serial should be %1 instead of %2").arg(player->m_expectedReplySerial).arg(packet->localSerial));
    else
        success = true;
...
bool Room::useCard(const CardUseStruct &use, bool add_history)
{
    ...
    bool slash_not_record = key.contains("Slash") && slash_count > 0
    ...
    try {
        ...
        if (triggerEvent == StageChange || triggerEvent == TurnBroken) {
            if (getCardPlace(card_use.card->getEffectiveId()) == Player::PlaceTable) {
                CardMoveReason reason(CardMoveReason::S_REASON_UNKNOWN, card_use.from->objectName(), QString(),
                                      card_use.card->getSkillName(), QString());
                if (card_use.to.size() == 1)
                    reason.m_targetId = card_use.to.first()->objectName();
                moveCardTo(card_use.card, NULL, Player::DiscardPile, reason, true);
            }
...
void Room::loseHp(ServerPlayer *victim, int lose)
{
    ...
    JsonArray args;
    args << victim->objectName();
    args << -lose;
    args << -1;
    doBroadcastNotify(S_COMMAND_CHANGE_HP, args);
...
void Room::loseMaxHp(ServerPlayer *victim, int lose)
{
    ...
    args << victim->objectName();
    args << -lose;
    doBroadcastNotify(S_COMMAND_CHANGE_MAXHP, args);
...
bool Room::changeMaxHpForAwakenSkill(ServerPlayer *player, int magnitude)
{
    ...
    return true; //(player->getMark("@waked") >= n);
}
...
void Room::marshal(ServerPlayer *player)
{
    ...
    doNotify(player, S_COMMAND_START_IN_X_SECONDS, QVariant());
...
void Room::startGame()
{
    ...
    foreach (ServerPlayer *player, m_players) {
        ...
        if (mode == "02_1v1")
            doBroadcastNotify(getOtherPlayers(player, true), S_COMMAND_REVEAL_GENERAL,
                              JsonArray() << player->objectName() << player->getGeneralName());
...
bool Room::notifyProperty(ServerPlayer *playerToNotify, const ServerPlayer *propertyOwner, const char *propertyName,
                          const QString &value)
{
    ...
    if (propertyOwner == playerToNotify)
        args << QSanProtocol::S_PLAYER_SELF_REFERENCE_ID;
    else
        args << propertyOwner->objectName();
    args << propertyName;
    args << real_value;
    return doNotify(playerToNotify, S_COMMAND_SET_PROPERTY, args);
}
...
bool Room::broadcastProperty(ServerPlayer *player, const char *property_name, const QString &value)
{
    ...
    args << property_name;
    args << real_value;
    return doBroadcastNotify(S_COMMAND_SET_PROPERTY, args);
}
    ...
    foreach (ServerPlayer *player, players) {
        ...
        for (int i = 0; i < cards_moves.size(); i++) {
            ...
            cards_moves[i].open = forceVisible
                ...
                || (cards_moves[i].from_place == Player::PlaceHand && cards_moves[i].from
                    && cards_moves[i].from->hasFlag("Global_HandOpen"))
                || (cards_moves[i].to_place == Player::PlaceHand && cards_moves[i].to
                    && cards_moves[i].to->hasFlag("Global_HandOpen"))
...
void Room::doAnimate(QSanProtocol::AnimateType type, const QString &arg1, const QString &arg2, QList<ServerPlayer *> players)
{
    ...
    args << arg1;
    args << arg2;
    doBroadcastNotify(players, S_COMMAND_ANIMATE, args);
}
...
void Room::preparePlayers()
{
    ...
    JsonArray args;
    args << QSanProtocol::S_GAME_EVENT_PREPARE_SKILL;
    doBroadcastNotify(QSanProtocol::S_COMMAND_LOG_EVENT, args);
}
...
void Room::setEmotion(ServerPlayer *target, const QString &emotion)
{
    ...
    args << (emotion.isEmpty() ? "." : emotion);
    doBroadcastNotify(S_COMMAND_SET_EMOTION, args);
}
...
QString Room::askForKingdom(ServerPlayer *player)
{
    ...
    QString result = Sanguosha->getKingdoms().first();
    AI *ai = player->getAI();
    if (ai)
        ...
        if (success && isString(clientReply)) {
            ...
            if (!Sanguosha->getKingdoms().contains(result))
                result = Sanguosha->getKingdoms().first();
        }
    }
...
bool Room::askForDiscard(ServerPlayer *player, const QString &reason, int discard_num, int min_num, bool optional,
                         bool include_equip, const QString &prompt, const QString &pattern)
{
    if (!player->isAlive())
        return false;
    tryPause();
    ...
    if (!optional) {
        ...
        if (card_num <= min_num) {
            ...
            if (card_num < min_num && !jilei_list.isEmpty()) {
                ...
                gongxinArgs << false;
                gongxinArgs << toJsonArray(jilei_list);
    ...
    if (ai) {
        ...
        JsonArray clientReply = player->getClientReply().value<JsonArray>();
        if (!success || (clientReply.size() > discard_num || clientReply.size() < min_num)
    ...
    if (reason == "gamerule") {
        CardMoveReason mreason(CardMoveReason::S_REASON_RULEDISCARD, player->objectName(), QString(),
                               dummy_card->getSkillName(), reason);
        throwCard(dummy_card, mreason, player);
    } else {
        CardMoveReason mreason(CardMoveReason::S_REASON_THROW, player->objectName(), QString(), dummy_card->getSkillName(),
                               reason);
        throwCard(dummy_card, mreason, player);
    }
...
const Card *Room::askForExchange(ServerPlayer *player, const QString &reason, int discard_num, int min_num, bool include_equip,
                                 const QString &prompt, bool optional, const QString &pattern)
{
    if (!player->isAlive())
        return NULL;
    tryPause();
...
int Room::doGongxin(ServerPlayer *shenlvmeng, ServerPlayer *target, QList<int> enabled_ids, QString skill_name)
{
    ...
    if (ai) {
        ...
        gongxinArgs << true;
        gongxinArgs << toJsonArray(target->handCards());
        gongxinArgs << toJsonArray(enabled_ids);
        bool success = doRequest(shenlvmeng, S_COMMAND_SKILL_GONGXIN, gongxinArgs, true);
        QVariant clientReply = shenlvmeng->getClientReply();
        if (!success || !isNumber(clientReply) || !target->handCards().contains(clientReply.toInt())) {
            shenlvmeng->tag.remove(skill_name);
            return -1;
        }
...
PindianMap Room::askForPindianRace(ServerPlayer *from, QList<ServerPlayer *> tos, const QString &reason)
{
    ...
    foreach (ServerPlayer *player, players) {
        const Card *c = NULL;
        JsonArray clientReply = player->getClientReply().value<JsonArray>();
        if (!player->m_isClientResponseReady || !isString(clientReply[0])) {
...
{
    JsonArray options = toJsonArray(player->getSelected()).value<JsonArray>();
    if (!Config.EnableBasara) {
        if (getLord())
...
bool Room::makeCheat(ServerPlayer *player)
{
    ...
    if (code == S_CHEAT_KILL_PLAYER) {
        ...
        if (!isString(arg[1]) || !isBool(arg[2]))
            return false;
...
{
    JsonArray args;
    args << toJsonArray(card_ids);
    args << toJsonArray(disabled_ids);

    m_fillAGarg = args;

    if (who)
        doNotify(who, S_COMMAND_FILL_AMAZING_GRACE, args);
    else
        doBroadcastNotify(S_COMMAND_FILL_AMAZING_GRACE, args);
}

void Room::takeAG(ServerPlayer *player, int card_id, bool move_cards, QList<ServerPlayer *> to_notify)
{
    if (to_notify.isEmpty())
        to_notify = getAllPlayers(true);

    JsonArray args;
    args << (player ? player->objectName() : QVariant());
    args << card_id;
    args << move_cards;
    ...
    if (player) {
        ...
        foreach (ServerPlayer *p, to_notify)
            doNotify(p, S_COMMAND_TAKE_AMAZING_GRACE, args);
...
void Room::showAllCards(ServerPlayer *player, ServerPlayer *to)
{
    ...
    gongxinArgs << player->objectName();
    gongxinArgs << false;
    gongxinArgs << toJsonArray(player->handCards());
...
void Room::retrial(const Card *card, ServerPlayer *player, JudgeStruct *judge, const QString &skill_name, bool exchange)
{
    ...
    move1.reason.m_extraData = QVariant::fromValue(judge);
    ...
    CardsMoveStruct move2(QList<int>(), judge->who, exchange ? player : NULL, Player::PlaceUnknown,
                          exchange ? Player::PlaceHand : Player::DiscardPile, reason);
                      ...
                      bool notify_skill)
{
    ...
    while (!remain_cards.isEmpty() && num > 0) {
        ...
        if (ai) {
            ...
            JsonArray args;
            args << toJsonArray(remain_cards);
            args << optional;
            args << num;
            QStringList player_names;
            foreach (ServerPlayer *player, players)
                player_names << player->objectName();
            args << toJsonArray(player_names);
            if (!prompt.isEmpty())
                args << prompt;
            bool success = doRequest(liubei, S_COMMAND_SKILL_YIJI, args, true);
            ...
            if (!tryParse(clientReply[0], ids) || !isString(clientReply[1]))
                break;
    ...
    QVariant decisionData
        = QString("Rende:%1:%2:%3:%4")
              .arg(skill_name, liubei->objectName(), namelist.join("+"), IntList2StringList(give_map.keys()).join("+"));
...
bool Room::askForYiji(ServerPlayer *guojia, QList<int> &cards, const QString &skill_name, bool is_preview, bool visible,
                      bool optional, int max_num, QList<ServerPlayer *> players, CardMoveReason reason, const QString &prompt,
                      bool notify_skill)
{
    if (max_num == -1)
        max_num = cards.length();
    ...
    if (ai) {
        ...
        if (!prompt.isEmpty())
            args << prompt;
        bool success = doRequest(guojia, S_COMMAND_SKILL_YIJI, args, true);
...
QString Room::askForRole(ServerPlayer *player, const QStringList &roles, const QString &scheme)
{
    ...
    JsonArray args;
    args << scheme;
    args << toJsonArray(squeezed);
    bool success = doRequest(player, S_COMMAND_CHOOSE_ROLE_3V3, args, true);
    QVariant clientReply = player->getClientReply();
