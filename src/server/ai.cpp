...
AI::Relation AI::GetRelationHegemony(const ServerPlayer *a, const ServerPlayer *b)
{
    ...
    const QString aName = aShown ? a->getGeneralName() : a->getRoom()->getTag(a->objectName()).toStringList().first();
    const QString bName = bShown ? b->getGeneralName() : b->getRoom()->getTag(b->objectName()).toStringList().first();
...
const Card *TrustAI::askForPindian(ServerPlayer *requestor, const QString &reason)
{
    ...
    // ikbiansheng special case
    if (reason == "ikbiansheng_pindian" && self->hasLordSkill("ikbiansheng"))
        return cards.last();
