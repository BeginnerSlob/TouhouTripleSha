...
version = "TouhouTripleSha AI 20160316 (V0.41)"
...
function sgs.getDefense(player)
	...
	if not attacker or (attacker and not attacker:hasSkill("ikxuwu")) then
	...
	if attacker and attacker:hasSkill("ikxuwu") and player:hasSkill("zhaxiang") then defense = defense + 4 end

	if player:hasSkill("nosrende") and player:getHp() > 2 and player:getHandcardNum() > 1 then defense = defense + 1 end
	if player:hasSkill("ikshenai") and player:getHp() > 2 and player:getHandcardNum() > 1 then defense = defense + 1 end
...
function SmartAI:assignKeep(start)
	if start then
		...
		for _, enemy in ipairs(self.enemies) do
			if enemy:hasSkill("ikwanhun") and enemy:distanceTo(self.player) == 1 then
...
function SmartAI:getDynamicUsePriority(card)
	...
	if self.player:hasSkill("nosdanshou") and not self.player:hasSkill("ikxuwu")
...
function SmartAI:objectiveLevel(player)
	...
	if self.role == "renegade" then
		...
		if target_role == "rebel" and player:getHp() <= 1 and not hasBuquEffect(player) and not player:hasSkill("ikjingyou")
...
function SmartAI:askForNullification(trick, from, to, positive, isHeg)
	...
	if from and not from:hasSkill("ikxuwu") then
		if (trick:isKindOf("Duel") or trick:isKindOf("FireAttack") or trick:isKindOf("Drowning") or trick:isKindOf("AOE"))
			and (to:hasSkill("ikmitu") or (self:getDamagedEffects(to, from) and self:isFriend(to))) then
			return nil
		end
	...
	if positive then
		...
		if from and self:isEnemy(from) and (sgs.evaluatePlayerRole(from) ~= "neutral" or sgs.isRolePredictable()) then
			if self.player:hasSkill("ikjingyou") and self.player:getHandcardNum() == 1 and self.player:isLastHandCard(null_card) and trick:isKindOf("SingleTargetTrick") then return null_card end
		...
		if self:isFriend(to) then
			...
			if trick:isKindOf("AOE") and not (from:hasSkill("ikmitu") and not (menghuo and trick:isKindOf("SavageAssault"))) then
				local lord = self.room:getLord()
			...
			if trick:isKindOf("Duel") and not from:hasSkill("ikmitu") then
				if self.player:objectName() == to:objectName() then
...
function sgs.ai_skill_cardask.nullfilter(self, data, pattern, target)
	...
	if target and target:hasSkill("ikxuwu") and not self:needToLoseHp() then return end
	if effect and target and target:hasSkill("ikwanhun") and target:distanceTo(self.player) == 1 then return end
...
function SmartAI:needKongcheng(player, need_keep)
	player = player or self.player
	if need_keep then
		return player:isKongcheng() and (player:hasSkill("ikjingyou") or (player:hasSkill("zhiji") and player:getMark("zhiji") == 0))
	end
...
function SmartAI:getCardNeedPlayer(cards, friends_table, handcard)
	...
	if (self.player:hasSkill("nosrende") and self.player:isWounded() and self.player:getMark("nosrende") < 2) then
		...
		if ((self.player:getHandcardNum() == 2 and self.player:getMark("nosrende") == 0) or
			...
			for _, enemy in ipairs(self.enemies) do
				...
				if enemy:canSlash(self.player) and enemy:hasSkill("ikwanhun") and enemy:distanceTo(self.player) == 1 then return end
			end
		end
	end

	if (self.player:hasSkill("ikshenai") and not self.player:hasUsed("IkShenaiCard") and self.player:isWounded()) then
		if (self.player:getHandcardNum() < 2) then return end

		if self.player:getHandcardNum() == 2 and self:getOverflow() <= 0 then
			for _, enemy in ipairs(self.enemies) do
				if enemy:hasWeapon("guding_blade")
					and (enemy:canSlash(self.player)
					or self.player:hasSkills("shensu|jiangchi|tianyi|wushen|thyuchang|nosgongqi")) then
	...
	-- slash
	if self.role == "lord" and self.player:hasLordSkill("ikxinqi") then
		for _, friend in ipairs(friends) do
			if friend:getKingdom() == "kaze" and friend:getHandcardNum() < 3 then
				for _, hcard in ipairs(cardtogive) do
					if isCard("Slash", hcard, friend) then
						return hcard, friend
					end
	...
	if #self.enemies > 0 and self.enemies[1]:isKongcheng() and self.enemies[1]:hasSkill("ikjingyou")
	...
	local shoulduse = self.player:isWounded()
						and self.player:hasSkill("ikshenai") and not self.player:hasUsed("IkShenaiCard")
	...
	if #cards > 0 and shouldUse then
		local need_ikshenai = (sgs.current_mode_players["rebel"] == 0 and sgs.current_mode_players["loyalist"] > 0 and self.player:isWounded())
...
sgs.ai_choicemade_filter.Yiji.general = function(self, from, promptlist)
	...
	if hasManjuanEffect(to) then
		intention = 0
	elseif to:hasSkill("ikjingyou") and to:isKongcheng() then
		intention = 30
	end
...
function SmartAI:ableToSave(saver, dying)
	...
	if current and current:getPhase() ~= sgs.Player_NotActive and current:hasSkill("wansha")
		...
		and not saver:hasSkills("jiuzhu|chunlao|ikjieyou|nosrenxin") then
		return false
	end
	local peach = sgs.cloneCard("peach", sgs.Card_NoSuitRed, 0)
	if saver:isCardLimited(peach, sgs.Card_MethodUse, true) and not saver:hasSkills("jiuzhu|chunlao|ikjieyou|nosrenxin") then return false end
	return true
end
...
function SmartAI:needRetrial(judge)
	...
	if reason == "lightning" then
		if who:hasSkills("ikmitu|hongyan") then return false end
...
function SmartAI:dontRespondPeachInJudge(judge)
	...
	if judge.reason == "ikyindie" and judge.who:getMark("ikguiyue") == 0 and judge.who:getPile("field"):length() < 2 then return true
...
function SmartAI:getDamagedEffects(player, damage_from, isSlash)
	...
	if isSlash then
		if attacker:hasSkill("ikwanhun") and attacker:distanceTo(player) == 1 and not self:isFriend(player, attacker) then
			return false
		end
...
function getCardsNum(class_name, player, from)
	...
	if class_name == "Slash" then
		local slashnum
		if player:hasSkill("ikchilian") then
...
function SmartAI:getCardsNum(class_name, flag, selfonly)
	...
	elseif class_name == "Slash" then
		if player:hasLordSkill("ikxinqi") then
...
function SmartAI:useBasicCard(card, use)
	...
	if self:needIkShenai() then return end
...
function SmartAI:aoeIsEffective(card, to, source)
	...
	if to:hasSkill("ikmitu") and not source:hasSkill("ikxuwu") then
		return false
	end
...
function SmartAI:getDistanceLimit(card, from)
	...
	if from:hasFlag("ThChouceUse") then
		return
	end
	if card:getSkillName() == "qiaoshui" or card:getSkillName() == "thpanghun" then
		return
	end
...
function SmartAI:useTrickCard(card, use)
	...
	if self:needIkShenai() and not card:isKindOf("ExNihilo") then return end
	if card:isKindOf("AOE") and not card:isKindOf("BurningCamps") then
		...
		if self.player:hasSkill("ikmitu")
			and not (menghuo and (not menghuo:hasSkill("ikmitu") or menghuo:hasSkill("ikxuwu")) and avail > 1)
			and not self.player:hasSkill("ikxuwu") then
...
function SmartAI:useEquipCard(card, use)
	...
	if card:isKindOf("Weapon") then
		...
		if self.player:hasSkill("nosrende") or (self.player:hasSkill("ikshenai") and not self.player:hasUsed("IkShenaiCard")) then
			for _, friend in ipairs(self.friends_noself) do
	...
	elseif card:isKindOf("Armor") then
		...
		if (self.player:hasSkill("nosrende") or (self.player:hasSkill("ikshenai") and not self.player:hasUsed("IkShenaiCard"))) and self:evaluateArmor(card) < 4 then
			for _, friend in ipairs(self.friends_noself) do
...
function SmartAI:needIkShenai()
	return self.player:getLostHp() > 1 and self:findFriendsByType(sgs.Friend_Draw)
			and ((self.player:hasSkill("nosrende") and self.player:getMark("nosrende") < 2)
				or (self.player:hasSkill("ikshenai") and not self.player:hasUsed("IkShenaiCard")))
end
...
function SmartAI:doNotDiscard(to, flags, conservative, n, reason)
	...
	for _, enemy in ipairs(enemies) do
		if not enemy:hasSkills("nosqianxun|weimu") then
			good_enemy = true
			break
		end
	end
	...
	if to:hasSkills("ikyindie+ikguiyue") and to:getPhase() == sgs.Player_NotActive and (conservative or (good_enemy and #self.enemies > 1)) then return true end
...
function SmartAI:findPlayerToDiscard(flags, include_self, isDiscard, players, return_table, pattern)
	...
	if flags:match("h") then
		for _, enemy in ipairs(enemies) do
			...
			if #cards <= 2 and not enemy:isKongcheng() and not (enemy:hasSkills("ikyindie+ikguiyue") and enemy:getPhase() == sgs.Player_NotActive) then
				for _, cc in ipairs(cards) do
...
function getBestHp(player)
	local arr = { ganlu = 1, yinghun = 1, nosmiji = 1, xueji = 1, baobian = 1, shangshi = 1, nosshangshi = 1 }
...
function SmartAI:needToLoseHp(to, from, isSlash, passive)
	from = from or self.room:getCurrent()
	to = to or self.player
	if isSlash and not from:hasSkill("ikxuwu") then
		if from:hasSkill("ikwanhun") and from:distanceTo(to) == 1 then
			return false
		end
	...
	if from:hasSkill("ikxuwu") and to:hasSkills(sgs.masochism_skill) then return false end
	...
	if not passive then
		if to:getMaxHp() > 2 then
			...
			if (to:hasSkill("nosrende") or (to:hasSkill("ikshenai") and not to:hasUsed("IkShenaiCard")))
