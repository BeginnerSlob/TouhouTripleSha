...
function SmartAI:useCardSupplyShortage(card, use)
	...
	for _, enemy in ipairs(enemies) do
		if (self:hasSkills("chenyan|haoshi|tuxi", enemy) or (enemy:hasSkill("zaiqi") and enemy:getLostHp() > 1)) and
	...
	for _, enemy in ipairs(enemies) do
		if ((#enemies == 1) or not self:hasSkills("tiandu|tianshi",enemy)) and not enemy:containsTrick("supply_shortage") and enemy:faceUp() and self:objectiveLevel(enemy) > 3 then
			use.card = card
			if use.to then use.to:append(enemy) end
...
function SmartAI:useCardFireAttack(fire_attack, use)  
	...
	or (self:isEquip("SilverLion") and self:hasSkill("huanji"))) and self.player:getHandcardNum() > 1 then
		use.card = fire_attack
