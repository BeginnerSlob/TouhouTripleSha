...
function sgs.ai_slash_prohibit.huilei(self, from, to)
	if not to:hasSkill("huilei") then return false end
...
function sgs.ai_slash_prohibit.enyuan(self, from, to)
	if not to:hasSkill("enyuan") then return false end
...
sgs.ai_skill_cardask["xuanhuo-slash"] = function(self, data, pattern, target, target2)
	...
	if target and target2 then
		...
		if self.player:hasSkills(sgs.lose_equip_skill) and not self.player:getEquips():isEmpty() and self.player:getHandcardNum() < 2 and not self.player:hasSkill("manjuan") then return "." end
		for _, slash in ipairs(self:getCards("Slash")) do
