-- translation for Kishin Package

return {
	["touhou-kishin"] = "东方的日辉",
	
	["#kishin001"] = "逆袭的天邪鬼",
	["kishin001"] = "鬼人正邪",--风 - 空 - 4血
	["designer:kishin001"] = "幻桜落 | Codeby:Slob",
	["illustrator:kishin001"] = "正体不明",
	["cv:kishin001"] = "向晚",
	["~kishin001"] = "善哉善哉...无上甚深圆满正等觉...",
	["thluanshen"] = "乱神",
	[":thluanshen"] = "出牌阶段限一次，若你的手牌数不小于体力值，你可以展示任意张手牌并让一名其他角色选择一项：弃置你展示的牌并弃置等量的牌；或获得你展示的牌，然后将手牌补至等同于其体力上限的张数并将其人物牌翻面。",
	["thluanshen:discard"] = "弃置其展示的牌",
	["thluanshen:turnover"] = "获得其展示的牌",
	["$thluanshen1"] = "休怪我做金刚怒目！",
	["$thluanshen2"] = "大胆！",
	
	["#kishin002"] = "柳下的辘轳首",
	["kishin002"] = "赤蛮奇",--花 - 空 - 4血
	["designer:kishin002"] = "幻桜落 | Codeby:Slob",
	["illustrator:kishin002"] = "正体不明",
	["cv:kishin002"] = "暂无",
	["thfeiman"] = "飞蛮",
	[":thfeiman"] = "出牌阶段开始时，你可以对一名其他角色造成1点伤害，然后令其选择一项：对其攻击范围内的一名角色造成一点火焰伤害，或获得场上的一张牌。",
	["@thfeiman"] = "你可以发动“飞蛮”",
	["thfeiman:damage"] = "对攻击范围内的一名角色造成1点火焰伤害",
	["thfeiman:obtain"] = "获得场上的一张牌",
	["thguaiqi"] = "怪奇",
	[":thguaiqi"] = "一名角色的出牌阶段结束时，若其此阶段内至少有一名角色受到了2点或更多的伤害，你可以令当前回合的角色摸一张牌。",
	
	["#kishin003"] = "秘境的人鱼",
	["kishin003"] = "若鹭姬",--雪 - 空 - 4血
	["designer:kishin003"] = "幻桜落 | Codeby:Slob",
	["illustrator:kishin003"] = "正体不明",
	["cv:kishin003"] = "暂无",
	["thjingtao"] = "惊涛",
	[":thjingtao"] = "每当你失去最后的手牌时，你可以摸两张牌。",
	["thzongni"] = "纵溺",
	[":thzongni"] = "你回合内的摸牌阶段开始时，你可以将此阶段视为出牌阶段，若如此做，此阶段结束时，你须弃置至少一张手牌。",
	["#thzongni"] = "纵溺（后续效果）",
	["@thzongni-discard"] = "受到“纵溺”效果的影响，你须弃置至少一张手牌",
	
	["#kishin004"] = "幻想琉璃双镜",
	["kishin004"] = "九十九弁弁＆九十九八桥",--月 - 空 - 4血
	["designer:kishin004"] = "幻桜落 | Codeby:Slob",
	["illustrator:kishin004"] = "正体不明",
	["cv:kishin004"] = "暂无",
	["thlanzou"] = "岚奏",
	[":thlanzou"] = "弃牌阶段结束时，若你于此阶段弃置了X张或更多的手牌，你可以将此阶段弃置的牌交给一名其他角色，然后其失去1点体力；若你在此阶段弃置了小于X张的手牌，你可以失去1点体力，然后弃置任意名其他角色的共计X张牌（X为你装备区的牌的数量，且至多为四）。",
	["@thlanzou"] = "你可以发动“岚奏”",
	["~thlanzou"] = "选择若干名角色（若弃置多张牌则多次点击）→点击“确定”",
	
	["#kishin005"] = "丧心的假面",
	["kishin005"] = "秦心",--风 - 幻 - 3血
	["designer:kishin005"] = "幻桜落 | Codeby:Slob",
	["illustrator:kishin005"] = "正体不明",
	["cv:kishin005"] = "暂无",
	["thxinqi"] = "心绮",
	[":thxinqi"] = "锁定技，当其他角色使用黑色非延时类锦囊牌指定你为目标时，需令你摸一张牌，且若其有手牌需令你观看之，否则该锦囊牌对你无效。",
	["thxinqi:show"] = "令其观看所有手牌",
	["thxinqi:cancel"] = "令该锦囊牌对其无效",
	["thnengwu"] = "能舞",
	["#thnengwu-clear"] = "能舞（移除使用限制）",
	[":thnengwu"] = "其他角色的准备阶段开始时，若你有手牌，你可以令该角色选择其一张手牌，然后你选择一种牌的类别并亮出该牌：若为你所选的类别，该角色须失去1点体力；否则你须弃置一张牌。该角色不可以使用其他的牌，直到该角色因使用、打出或弃置而失去这张手牌或回合结束。",
	["#ThNengwu"] = "%from 选择了 %arg",
	
	["#kishin006"] = "小人的末裔",
	["kishin006"] = "少名针妙丸",--花 - 空 - 3血
	["designer:kishin006"] = "幻桜落 | Codeby:Slob",
	["illustrator:kishin006"] = "正体不明",
	["cv:kishin006"] = "暂无",
	["thbaochui"] = "宝锤",
	[":thbaochui"] = "其他角色的摸牌阶段结束时，你可令其交给你一张手牌，然后你将全部的手牌置于你的人物牌上，若如此做，该角色可于此出牌阶段将你人物牌上的牌如手牌般使用或打出，且此出牌阶段结束时，你获得你人物牌上全部的牌，并摸一张牌。",
	["@thbaochui"] = "%src 对你发动了“宝锤”，请交给其一张手牌",
	["#thbaochui-return"] = "宝锤（获得牌）",
	["thbaochuipile"] = "宝锤",
	["thyishi"] = "倚势",
	[":thyishi"] = "锁定技，你人物牌上的牌对你无效。",
	
	["#kishin007"] = "幻梦的演奏家",
	["kishin007"] = "堀川雷鼓",--雪 - 空 - 4血
	["designer:kishin007"] = "幻桜落 | Codeby:Slob",
	["illustrator:kishin007"] = "正体不明",
	["cv:kishin007"] = "暂无",
	["thmoju"] = "魔具",
	[":thmoju"] = "锁定技，摸牌阶段，若你的装备区有武器牌，你摸牌的数量改为你攻击范围的数量（至少摸两张）；出牌阶段，当你使用【杀】指定一名角色为目标后，若你的装备区有防具牌，此【杀】不可以被【闪】响应；你的装备区每有一张坐骑牌，你的手牌上限便+1。",
	
	["#kishin008"] = "月下的独狼",
	["kishin008"] = "今泉影狼",--月 - 空 - 4血
	["designer:kishin008"] = "幻桜落 | Codeby:Slob",
	["illustrator:kishin008"] = "正体不明",
	["cv:kishin008"] = "暂无",
	["thlianying"] = "莲影",
	[":thlianying"] = "出牌阶段限一次，你可以弃置两张牌，若如此做，你获得技能“赤莲”和“疾步”，直到回合结束。",
	["thyuanxiao"] = "远哮",
	[":thyuanxiao"] = "每当你使用【杀】指定一名目标角色后，若该角色的手牌数大于你，你可以获得其一张手牌并展示之，若此牌为红色，其不能使用【闪】抵消此【杀】。",
}
