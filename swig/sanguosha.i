...
{
public:
    enum Phase {RoundStart, Start, Judge, Draw, Play, Discard, Finish, RoundEnd, NotActive, PhaseNone};
...
public:
    ...
    static const int S_REASON_EXCHANGE_FROM_PILE = 0x39;// exchange cards from cards moved out of game (for qiyao only)
...
enum TriggerEvent{
    ...
    DamageForseen,    // the first event in a damage -- liefeng miaowu
...
public:
    ...
    void doLingshi(ServerPlayer *shenlumeng, ServerPlayer *target);
    int drawCard();
    const Card *peek();
